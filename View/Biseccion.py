import numpy as np
from PyQt5.QtWidgets import QVBoxLayout, QWidget, QLabel, QTextEdit, QPushButton, QHBoxLayout, QSpacerItem, QSizePolicy,QScrollArea
...
import Model.Biseccion as biseccionModel

class BiseccionPanel(QWidget):
    def __init__(self):
        super().__init__()
        self.initialize()
    ...
    def __sendData(self):
        ...
        if '' in data:
            ...
            error, tabla, respuesta, iteracion = biseccionModel.proceso(data)
            if "Error" in str(respuesta) or "No tiene raíz" in str(respuesta):
                self.__errorMsm.setAlignment(Qt.AlignLeft | Qt.AlignTop)
                self.__errorMsm.setWordWrap(True)
                self.__errorMsm.setStyleSheet("color: red")
                ...
                self.__errorMsm.setAlignment(Qt.AlignLeft | Qt.AlignTop)
                self.__errorMsm.setWordWrap(True)
                resulatdo = "Raíz: "+ str(respuesta)+ "\nError: " + str(error)+"\n[        xi                   xu                   xr                 fxi             fxr               f(xi)*f(xr)      ]\n" + str(tabla)
                ...
                self.__errorMsm.setText(resulatdo)
                self.__errorMsm.setFixedSize(1000, (iteracion*35))
