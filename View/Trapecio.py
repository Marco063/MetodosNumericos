...
import Model.Trapecio as trapecioModel

class TrapecioPanel(QWidget):
    def __init__(self):
        super().__init__()
        self.__initialize()
    ...
    def __sendData(self):
        ...
        if '' in data:
            ...
            resultado, error, lista = trapecioModel.proceso(data)
            if "Error" in str(resultado):
                self.__errorMsm.setStyleSheet("color: red")
                self.__errorMsm.setText(resultado)
            else:
                self.__showGraph(data, lista, resultado)
                self.__canvas.draw()
    ...
    def __showGraph(self, data, lista, resultado):
        self.__figure.clear()
        ax = self.__figure.add_subplot(111)
